...
const insertFirstParagraphAsText: typeRule = (
    ...
    opts.firstNodeAsText = false;

    // If the startBlock is void
    const { document } = change.value;
    const ancestors = document.getAncestors(range.startKey);
    const voidParent = document.findLast(n => n.isVoid);
    if (voidParent) {
        const container = ancestors.find(
            (n, index) => voidParent === ancestors.get(index + 1)
        );

        const nextText = container.getNextText(voidParent.key);
        if (!nextText) {
